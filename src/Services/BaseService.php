...
//        }
    ...
    {
        if ($this->isSite() || config('app.seeding')) {
            return true;
        }
    ...
    public function search(Collection $input)
    {
        ...
        if (!InputHelper::null($input, 'eagers')) {
            $this->eagers = array_unique(array_merge($this->eagers, $input->get('eagers')));
        }
        ...
        if ($this->repo->getModel()->hasAttribute('alias') && $this->repo->getModel()->getTable() != 'extrafields') {
            ...
            if ($this->repo->getModel()->hasAttribute('language')) {
                if (InputHelper::null($input, 'language')) {
                    $same = $this->findByChain(['alias', 'language'],
                        ['=', '='],
                        [$input->get('alias'), config('daydreamlab.global.locale')]
                    )->first();
                } else {
                    $same = $this->findByChain(['alias', 'language'],
                        ['=', '='],
                        [$input->get('alias'), $input->get('language')]
                    )->first();
                }
            } else {
    ...
    {
        if ($item->locked_by && $item->locked_by != $this->user->id && !$this->user->higherPermissionThan(
                $item->locked_by
            )) {
